...
def supervisor_node(state: AgentState) -> AgentState:
    ...
    Routing priority: human_review > policy_tool_worker > retrieval_worker (explicit) > retrieval_worker (default)
    """
    task = state["task"].lower()
    state["history"].append(f"[supervisor] received task: {state['task'][:80]}")

    # Routing priority (từ cao → thấp):
    #   1. Unknown error code (ERR-xxx)      → human_review
    #   2. Policy / access / refund keywords → policy_tool_worker
    ...
    # Risk signals — check trước, ảnh hưởng route_reason cuối
    risk_keywords = ["khẩn cấp", "emergency", "2am", "2 am", "urgent"]
    if any(kw in task for kw in risk_keywords):
        risk_high = True

    # Priority 1: Unknown error code → human_review
    if re.search(r"\berr-\w+", task):
        route = "human_review"
        route_reason = "unknown error code (ERR-xxx) detected → human review"
        risk_high = True

    # Priority 2: Policy / access / refund → policy_tool_worker
    ...
    # Priority 3: SLA / ticket / escalation → retrieval_worker (explicit)
    elif any(kw in task for kw in ["p1", "escalation", "sla", "ticket", "helpdesk"]):
        matched = next(kw for kw in ["p1", "escalation", "sla", "ticket", "helpdesk"] if kw in task)
...
def policy_tool_worker_node(state: AgentState) -> AgentState:
    ...
    # Sprint 2: Thay bằng policy_tool_run(state)
    return policy_tool_run(state)


def synthesis_worker_node(state: AgentState) -> AgentState:
    """Wrapper gọi synthesis worker."""
    # Sprint 2: Thay bằng synthesis_run(state)
...
def build_graph():
    ...
    def run(state: AgentState) -> AgentState:
        ...
        if route == "human_review":
            ...
            # Policy worker needs context. Try retrieval first if not already done.
