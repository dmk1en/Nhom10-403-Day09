...
try:
    ...
    app = FastAPI(
        ...
        description="Day 09 Lab - MCP Server with Bonus HTTP Interface"
    )
    ...
    def call_tool(request: ToolCallRequest):
        ...
        if isinstance(result, dict) and "error" in result and "schema" in result:
...
if __name__ == "__main__":
    ...
    if args.server:
        ...
        uvicorn.run("mcp_server:app", host="0.0.0.0", port=args.port, reload=False)
    else:
        print("=" * 60)
        ...
        print("\nTo run the HTTP server: python mcp_server.py --server")
